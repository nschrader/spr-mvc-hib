...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title><spring:message code="nav.member.add"/></title>
  ...
  <%@include file="navbar.jsp" %>
	...
	<h1><spring:message code="nav.member.add"/></h1>
	<p><spring:message code="amf.description"/></p>
	<p>${message}</p>
	<form:form method="post" commandName="member" action="${pageContext.request.contextPath}/member/add.html" onsubmit="if (this.name.value == '' || this.teams.options.selectedIndex == -1) {return false;}">
	  <table class="table">
	    <tbody>
	      <tr>
		<td><spring:message code="lom.table.c2"/>:</td>
		<td><form:input path="name" /></td>
	      </tr>
	      <tr>
		<td><spring:message code="lom.table.c4"/>:</td>
		<td>
		  <form:select path="teams">
		    <form:options items="${allTeams}" multiple="true" itemValue="id" itemLabel="name" />
		  </form:select>
		</td>
	      </tr>
	      <tr>
		<td>
		  <input type="submit" value="<spring:message code="amf.submit"/>"/>
		</td>
		<td></td>
	      </tr>
	    </tbody>
